...
stemmer_path <- normalizePath("C:/Users/lukas/Dropbox/Članci/NewAge/NewAge-Channels-on-YouTube/stemmer/stemmer.R", winslash = "/", mustWork = TRUE)
source(stemmer_path, encoding = "UTF-8")
```
...
word_freq_data <- n_tokenTidy %>%
  filter(channelTitle %in% c("Anita Luis Osobni razvoj", 
                             ...
                             "SAFARI DUHA", 
                             "Put u središte sebe")) %>%
  group_by(channelTitle, word) %>%
  count() %>%
  filter(n > 10) %>%
  ungroup()

# Create a plot with facets for each channelTitle
ggplot(word_freq_data, aes(x = reorder(word, n), y = n)) +
  geom_col() +
  coord_flip() +
