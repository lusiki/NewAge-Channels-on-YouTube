...
```{r echo=F, eval=F, message=F , warning= FALSE, fig.height=25}
# ## Most common words
...
autorstvo <- data.frame(
  ...
  text = c(
    ...
    "Sponzorirani sadržaj / PR sadržaj",
    "Nativni tim / studio",
...
#  scale_fill_discrete(name = "Sentiment")+
  ...
  breaks = c("1", "2", "3"),
  labels = c("1 = Pozitivan", "2 = Neutralan", "3 = Negativan")) +  # Assuming two sentiment values; adjust as needed
  theme(
...
#  scale_fill_discrete(name = "Sentiment")+
  scale_fill_manual(values = c("grey10", "grey30", "grey50", "grey70"), name = "Sentiment",
  breaks = c("1", "2", "3"),
  labels = c("1 = Pozitivan", "2 = Neutralan", "3 = Negativan")) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(color = "black"),
    axis.text.y = element_text(color = "black"),
...
filtered_data <- gg1 %>%
  filter(INDUSTRIJA == "Zdravstvena industrija")
...
ggplot(sentiment_counts, aes(x = factor(SENTIMENT), y = PercentTotal, fill = factor(SENTIMENT))) +
  ...
  labs(
    ...
    title = "Sentiment Distribution within Zdravstvena industrija Category"
  ) +
  scale_fill_manual(
...
```{r echo=F, eval=F, message=F , warning= FALSE, fig.height=15, fig.width=15}
...
```{r  echo=F, eval=F, message=F , warning= FALSE, fig.height=15, fig.width=20}
gg4 <- dta %>% 
...
```{r echo=F, eval=F, message=F , warning= FALSE, fig.height=50, fig.width=15}
gg5 <- stemmed %>%
...
proba <- stemmed %>%
  ...
  group_by(V1) %>%
  count(sentiment) %>% 
  mutate(sentiment = case_when(sentiment == 0 ~ "Neutralan",
                                 sentiment == 1 ~ "Negativan",
                                 sentiment == 2 ~ "Pozitivan")) %>%
  spread(sentiment, n) %>%
...
```{r echo=F, eval=F, message=F , warning= FALSE, fig.height=15}
stemmed %>%
  filter(INDUSTRIJA == 11) %>%
  inner_join(NRCpn,by="word") %>% 
  count(word, Emotion) %>% 
